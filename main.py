...
# ★ 국내 뉴스 필터링의 핵심: 검증된 국내 언론사 리스트 (IT, 보안, 종합)
TARGET_DOMAINS = [
    "yna.co.kr",        # 연합뉴스
    "etnews.com",       # 전자신문
    "zdnet.co.kr",      # 지디넷코리아
...
def search_news(query):
    ...
    response = tavily.search(
        ...
        max_results=30,  # 30개 정도면 충분
        include_domains=TARGET_DOMAINS, # ★ 핵심: 위에서 정의한 국내 사이트에서만 검색
        days=2
    )
...
# 5. AI 상세 요약 (안전 필터 해제 + 강력한 JSON 파싱)
def summarize_news(news_list):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            result = json.loads(text[start_index:end_index])
            return result
        else:
...
if __name__ == "__main__":
    try:
        # 키워드는 이제 search_news 함수 안에서 처리됨
